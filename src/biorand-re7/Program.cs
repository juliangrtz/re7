    ...
    public static int Main(string[] args) {
        ...
        app.Configure(config => {
            ...
            config.AddCommand<GenerateCommand>("generate")
                ...
                .WithExample("generate", "-o", "re_chunk_000.pak.patch_001.pak", "--seed", "35825", "--config",
                    "tough.json");
            config.AddCommand<SetupCommand>("setup")
                .WithDescription("Create a mini pak containing all the required vanilla assets.")
                .WithExample("setup", "-o", "custom.pak", "-i",
                    "C:\\Program Files (x86)\\Steam\\steamapps\\common\\RESIDENT EVIL 7 biohazard");
            config.AddCommand<UpdateCommand>("update")
                .WithDescription("Updates csv file(s).")
                .WithExample("update");
            config.AddCommand<ModCommand>("mod")
                .WithDescription(
                    "Export one or more standalone mods or combine them into a super mod. Run with no arguments to display available mods.")
                .WithExample("mod", "-m", "flamethrower", "-o", "mods", "-i",
                    "C:\\Program Files (x86)\\Steam\\steamapps\\common\\RESIDENT EVIL 7 biohazard")
                .WithExample("mod", "-m", "flamethrower", "-o", "supermod.zip", "-i",
                    "C:\\Program Files (x86)\\Steam\\steamapps\\common\\RESIDENT EVIL 7 biohazard");
        });
        return app.Run(args);
    }

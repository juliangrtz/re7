...
public partial class REFPlugin
{
    ...
    private static REFrameworkNET.Collections.IList<REFrameworkNET.Collections.IList<via.GameObject>>? GetManagedObjects(ObjectManager? objectManager)
    {
    ...
    private static ShellManager? TryGetShellManager(via.GameObject? playerObject)
    {
        ...
        try
        {
            ...
            var shellManagerObject = objectManager?.findObject("ShellManager") ?? ObjectManager.findObjectInCurrentScene("ShellManager");
    ...
    private static bool TryRequestEm3300BombExplosion(via.GameObject enemyObject)
    {
        try
        {
            ...
            {
                logger.Log("Unable to create Em3300 bomb explosion because the player, ShellManager, or enemy transform was unavailable.", isVerbose: true);
                return false;
            }
    ...
    private static bool TryRequestEm3300ExplosionEffect(via.GameObject enemyObject)
    {
        ...
        {
            logger.Log($"Unable to request Em3300 fallback explosion effect: {ex.GetType().Name}: {ex.Message}", isVerbose: true);
            return false;
        }
    }
    ...
    private static void DetonateEm3300(via.GameObject enemyObject)
    {
        ...
        {
            TryRequestEm3300ExplosionEffect(enemyObject);
        }
    ...
    private static void DespawnEm3300(via.GameObject enemyObject)
    {
        ...
        {
            logger.Log($"Unable to deactivate Em3300 object 0x{enemyObject.Address():X}: {ex.GetType().Name}: {ex.Message}", isVerbose: true);
        }
        ...
        {
            logger.Log($"Unable to destroy Em3300 object 0x{enemyObject.Address():X}: {ex.GetType().Name}: {ex.Message}", isVerbose: true);
        }
    }

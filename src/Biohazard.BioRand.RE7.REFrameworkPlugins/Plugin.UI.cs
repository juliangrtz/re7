...
public partial class REFPlugin {
    ...
    private static void DrawFeatureInfo() {
        ...
        DrawLabelValue("Reload speed",
            $"{FormatEnabled(Config.ReadOrDefault("weapon-mod-reload-speed", false))} ({GetWeaponReloadSpeedStateLabel()})");
    ...
    private static void DrawRandomEventDebugTools() {
        if (ImGui.TreeNode("Random event effects")) {
            ...
            DrawLabelValue("Status duration",
                string.Create(CultureInfo.InvariantCulture,
                    $"{GetRandomEventDurationSeconds(RandomEventKind.PlayerStatus):0.###}s"));

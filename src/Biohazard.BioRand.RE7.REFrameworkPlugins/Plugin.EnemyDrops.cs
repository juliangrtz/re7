...
public partial class REFPlugin {
    ...
    private static string GetEnemyDropRatioKey(string itemDataId) =>
        $"enemy-drop-ratio-{itemDataId.ToLowerInvariant()}";
    ...
    private static double GetEnemyDropMultiplier(ManagedObject dropSourceObject, via.GameObject? enemyObject,
        out string? enemyTypeId) {
        var runtimeTypeName = GetManagedObjectRuntimeTypeName(dropSourceObject);
    ...
    private static List<EnemyDropCandidate> BuildEnemyDropCandidates(Random rng, bool restrictToBossDropPool) {
        ...
        if (Config.ReadOrDefault("allow-dlc-items", false)
            && ReadEnemyDropConfigOrDefault("enemy-drop-valuable-birthday-skill", "item-drop-valuable-birthday-skill",
                false)) {
    ...
    private static bool TryGetEnemyDropWallClearanceDirection(via.GameObject enemyObject, via.vec3 dropPosition,
        out via.vec3 direction) {
    ...
    private static bool TryProjectEnemyDropAwayFromWall(via.GameObject enemyObject, via.vec3 dropPosition,
        out via.vec3 groundPosition) {
        groundPosition = dropPosition;
    ...
    private static void SpawnEnemyDrop(via.GameObject enemyObject, string? enemyTypeId, string itemDataId,
        int stackNum) {
        ...
        Logger.Log($"Spawned enemy drop '{itemDataId}' x{stackNum} for enemy object 0x{enemyObject.Address():X}.",
            isVerbose: true);
    }

    private static void SpawnConfiguredEnemyDrop(ManagedObject dropSourceObject, via.GameObject enemyObject,
        int generation) {
        ...
        if (selection == null) {
            Logger.Log($"No eligible enemy drop candidates for enemy object 0x{enemyObject.Address():X}.",
                isVerbose: true);
            return;
        }

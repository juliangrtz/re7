...
internal class WeaponDefinitionGenerator : IFileGenerator
{
    ...
    private readonly Regex _pfbRegex = new Regex("natives/stm/prefab/weapon.*_item.pfb.*", RegexOptions.Compiled);

    private readonly Dictionary<string, string> _nameLookup = new();
    private readonly List<string> _pakPaths = new();
    ...
    private readonly Regex wpIdRegex = new Regex(@"wp\d*", RegexOptions.Compiled | RegexOptions.IgnoreCase);
    ...
    private Dictionary<string, WeaponDamageStats> GetDamageStats(List<string> rcolPaths, string id, bool isDefaultBulletWeapon)
    {
        ...
        foreach (var rcolPath in rcolPaths)
        {
            ...
            foreach (var requestSet in requestSets)
            {
                if (requestSet.UserData.Type.Name == "app.Collision.AttackUserData")
                {
                    ...
                    {
                        result.Add($"{Path.GetFileNameWithoutExtension(rcolPath)}/{requestSet.Name}", new()
                        {
        ...
        foreach (var path in _pakPaths)
        {
            ...
            if (weaponComponent != null)
            {
                ...
                result.Add(new WeaponDefinition
                {
                    ...
                    Id = id,
                    IsGun = false,
                    Name = _nameLookup.GetValueOrDefault(weaponComponent.WeaponID.ToString(), null),
            ...
            else if (weaponGunComponent != null)
            {
                ...
                var rcolPaths = GetRcolPaths(id);
                var damage = GetDamageStats(rcolPaths, weaponGunComponent.WeaponID.ToString(), rcolPaths.Any(p => p.Contains("defaultbullet")));
                var userParamsPath = weaponGunComponent.WeaponGunParameter.Path.EmptyToNullStr();
                var userParamsPathFull = userParamsPath == null ? null : $"natives/stm/{userParamsPath.ToLowerInvariant()}.{FileVersions.UserFileVersion}";
                ...
                if (userParamsPath != null)
                {
                    ...
                    weaponParams = RszSerializer.Deserialize<WeaponGunParameter>(weaponParamsFile.GetObjects(_rszRepository)[0]);
                }
                ...
                result.Add(new WeaponDefinition
                {
                    ...
                    Id = id,
                    Name = _nameLookup.GetValueOrDefault(weaponGunComponent.WeaponID.ToString(), null),
                    IsGun = true,

...
internal static class KeyItemRouteGraphImageRenderer {
    ...
    public static byte[] Render(KeyItemLocationModifier.KeyItemRouteGraphDiagram diagram) {
        ...
        canvas.DrawText(
            "Edge labels are required key items; routing places each key before the labeled edge is crossed",
            width / 2f, 84, SKTextAlign.Center, subtitleFont, subtitlePaint);
    ...
    private static void DrawNode(
        ...
        for (var i = 0; i < lines.Length; i++) {
            canvas.DrawText(lines[i], rectangle.MidX, firstBaseline + (i * lineHeight), SKTextAlign.Center, font,
                textPaint);
        }
    }

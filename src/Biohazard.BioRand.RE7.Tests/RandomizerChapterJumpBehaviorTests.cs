...
public class RandomizerChapterJumpBehaviorTests
{
    ...
    public void ChapterJumpData_SkipGuestHouse_ChangesGuestHouseJumpToChapter3()
    {
        ...
        var before = RandomizerTestHelpers.GetChapterJump(result.ReadBeforeScene(RandomizerTestPaths.ChapterJumpScenePath), RandomizerTestPaths.GuestHouseJumpGuid);
        var after = RandomizerTestHelpers.GetChapterJump(result.ReadAfterScene(RandomizerTestPaths.ChapterJumpScenePath), RandomizerTestPaths.GuestHouseJumpGuid);
    ...
    public void ChapterJumpData_ShuffleWithoutFoundFootage_DerangesMainCampaignTransitions()
    {
        ...
        var before = RandomizerTestHelpers.GetChapterJumps(result.ReadBeforeScene(RandomizerTestPaths.ChapterJumpScenePath))
            .Where(x => candidates.Contains(x.JumpChapter))
            .ToArray();
        var after = RandomizerTestHelpers.GetChapterJumps(result.ReadAfterScene(RandomizerTestPaths.ChapterJumpScenePath))
